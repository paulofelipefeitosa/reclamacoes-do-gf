...
```{r}
sum_feel <- ggplot(union[which(union$sentimento < 20),], aes(x = sentimento, y = m_aval)) +
    geom_point() +
...
mean_lex <- union %>%
    ...
    summarise(real_gi = m_aval,
              pred_gi = (1 - sigmoid(mean_sent_per_word, a = 1.5, b = 0))*5 + 0.3,
              nosig = mean_sent_per_word)
```
...
norm_func_meanlex_plot <- mean_lex %>% 
    ggplot(aes(x = nosig, y = pred_gi)) + 
    geom_point() +
    #geom_jitter(alpha = .5)  +
    scale_y_continuous(limits = c(1, 5), breaks = c(1:5)) + 
    labs(x = "Média Lexicos",
         y = "Pred GI") +
    facet_wrap(~ Type)
